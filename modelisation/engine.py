...
class TourEnCours:
    ...
    def apply_effects(self, carte, target=None):
        ...
        if player.hero.weapon is not None and "aura" in player.hero.weapon.effects:
            if "damage" in player.hero.weapon.effects["aura"]:
                if "conditional" in player.hero.weapon.effects["aura"] and "if_spell" in player.hero.weapon.effects["aura"] and carte.type == "Sort":
                    adv.hero.damage(player.hero.weapon.effects["aura"][2])
        ...
        if carte.type == "Serviteur":
            ...
            if "cri de guerre" in carte.effects and not carte.is_dead():
                if "play" in carte.effects["cri de guerre"] and carte.effects["cri de guerre"][2] != "":
                    ...
                    if played_card.type == "Serviteur" and not len(player.servants) + len(player.lieux) == 7:
                        ...
                        if carte.effects["cri de guerre"][1][0] == "tous":
                            if "ennemi" in carte.effects["cri de guerre"][1]:
                                if "aléatoire" in carte.effects["cri de guerre"][1]:
                                    ...
                                    for card in [adv.hero] + adv.servants.cards:
                                        if [x.name for x in player.servants] and "Enchanteur" in [x.name for x in
                ...
                if "alextrasza" in carte.effects["cri de guerre"] and target is not None:
                    if target in [adv.hero] + adv.servants.cards:
                        target.damage(carte.effects["cri de guerre"][2])
                    else:
                        target.heal(carte.effects["cri de guerre"][2])
                        ...
                        if "conditional" in carte.effects["cri de guerre"][1]:
                            ...
                            if "sort" in carte.effects["cri de guerre"][2]:
                                ...
                                if 2 in carte.effects["cri de guerre"][2]:
                                    self.plt.cards_chosen = self.choice_decouverte(type="sort") + self.choice_decouverte(type="sort")
                                else:
                                    self.plt.cards_chosen = self.choice_decouverte(type="sort")
                                ...
                                self.plt.cards_chosen = self.choice_decouverte(card_group=[get_card(x.name, all_spells) for x in carte.effects["cri de guerre"][2]])
                            if "adv_hand" in carte.effects["cri de guerre"][2] and "player_hand" in carte.effects["cri de guerre"][2]:
                                if player.hand.cards and adv.hand.cards:
                                    self.plt.cards_chosen = self.choice_decouverte(card_group=adv.hand) + self.choice_decouverte(card_group=player.hand)
            ...
            if "réincarnation" in carte.effects and carte.is_dead():
                if carte.effects["réincarnation"]:
            ...
            if "rale d'agonie" in carte.effects and carte.is_dead():
                ...
                if "boost" in carte.effects["rale d'agonie"] and target is not None:
                    if type(target) == Card:
                        ...
                        if "provocation" in carte.effects["rale d'agonie"][2]:
                            target.effects["provocation"] = 1
                if "heal" in carte.effects["rale d'agonie"] and target is not None:
                ...
                if "murmegivre" in carte.effects["rale d'agonie"]:
                    if ("allié" in carte.effects["rale d'agonie"][1] and carte in player.servants) or (
                            "ennemi" in carte.effects["rale d'agonie"][1] and carte in adv.servants):
                        player.hero.effects["murmegivre"] = 3
                    elif ("ennemi" in carte.effects["rale d'agonie"][1] and carte in player.servants) or (
                            "allié" in carte.effects["rale d'agonie"][1] and carte in adv.servants):
                        adv.hero.effects["murmegivre"] = 3
            if "rale d'agonie2" in carte.effects and carte.is_dead():
                ...
                if "boost" in carte.effects["rale d'agonie2"] and target is not None:
                    ...
                    if "provocation" in carte.effects["rale d'agonie2"][2]:
                        target.effects["provocation"] = 1
                if "heal" in carte.effects["rale d'agonie2"] and target is not None:
            ...
            if "damage" in carte.effects:
                if "tous" in carte.effects["damage"][0]:
                    if "except_self" in carte.effects["damage"][0]:
                        for entity in [x for x in [player.hero, adv.hero] + player.servants.cards + adv.servants.cards if x!= carte]:
                            entity.damage(carte.effects["damage"][1])
            if "add_mana" in carte.effects:
                player.mana += carte.effects["add_mana"]
            if "return_hand" in carte.effects and target is not None:
                adv.servants.remove(target)
                adv.hand.add(target)
                ...
                if type(carte.effects["add_hand"]) == CardGroup:
                    ...
                    if "sort" in carte.effects["add_hand"][0]:
                        if "until_full" in carte.effects["add_hand"][0]:
                            while len(player.hand) < 10:
                                player.hand.add(get_card(**random.choice([x for x in all_spells if x["decouvrable"] == 1])))
            ...
            if "boost" in carte.effects and target is not None:
                target.attack += carte.effects["boost"][0][0]
                target.base_attack += carte.effects["boost"][0][0]
                target.health += carte.effects["boost"][0][1]
                target.base_health += carte.effects["boost"][0][1]
                if "start_turn" in carte.effects["boost"][1]:
                    target.effects["aura"] = ["suicide", [""], "start_turn"]
            ...
            if "random_spell" in carte.effects["use"]:
                try:
                    for i in range(carte.effects["use"][2]):
                        ...
                        possible_targets = generate_targets(self.plt)[16 * (len(player.hand) - 1): 16 * len(player.hand) - 1]
                        possible_targets_refined = [n for n in range(len(possible_targets)) if possible_targets[n]]
                        if target == player.hero and 1 in possible_targets_refined:
                            final_target = target
                        elif target in player.servants and 2 in possible_targets_refined:
                            final_target = target
                        elif target == adv.hero and 8 in possible_targets_refined:
                            final_target = target
                        elif target in adv.servants and 9 in possible_targets_refined:
                            final_target = target
                        else:
                            ...
                            if final_target == 0:
                                ...
                                final_target = player.hero
                            elif final_target < 8:
                                final_target = player.servants[final_target - 2]
                            elif final_target == 9:
                                final_target = adv.hero
                            else:
        ...
        if carte.cost <= player.mana:
            if carte.type.lower() == "sort":
                ...
                if "counter" in [x.effects["aura"][0] for x in adv.servants if "aura" in x.effects] and "sort" in [x.effects["aura"][1] for x in adv.servants if "aura" in x.effects]:
                    ...
                    if [x for x in player.hand if "cri de guerre" in x.effects and "if_spell" in x.effects["cri de guerre"]]:
                        for serv in [x for x in player.hand if "if_spell" in x.effects["cri de guerre"]]:
    ...
    def attaquer(self, attaquant, cible, target=None):
        ...
        if type(attaquant) in (Hero, Card) and type(cible) in (Hero, Card):
            ...
            if "vol de vie" in attaquant.effects and "bouclier divin" not in cible.effects:
                player.hero.heal(attaquant.attack)
            if not "insensible_attack" in attaquant.effects:
                attaquant.damage(cible.attack)
            if "vol de vie" in cible.effects and "bouclier divin" not in attaquant.effects and "insensible_attack" not in attaquant.effects:
                adv.hero.heal(cible.attack)
    ...
    def decouverte(self, cards, choice, type_dec="decouverte"):
        if len(self.plt.players[0].hand) < 10 and type_dec == "decouverte":
            if choice != 3:
                ...
                elif self.plt.players[0].servants[-1].name == "Theotar le duc fou":
        ...
        if action == 0:
            ...
            if (action - 1) % 16 == 0:
                ...
                if "cri de guerre" in played_card.effects and "voisin" in played_card.effects["cri de guerre"][1] and len(adv.servants) > 1:
                    try:
                        if (action - 1) % 16 - 9 != 0:
                            target = CardGroup([target, adv.servants[(action) % 16 - 9], adv.servants[(action - 2) % 16 - 9]])
                    except:
                        try:
                            target = CardGroup([target] + [adv.servants[(action) % 16 - 9]])
                        except:
                            try:
                                target = CardGroup([target] + [adv.servants[(action - 2) % 16 - 9]])
                            except:
                                pass
            ...
            if plateau.cards_chosen:
                TourEnCours(plateau).decouverte(plateau.cards_chosen[0], action - 241)
            elif plateau.cards_dragage:
                TourEnCours(plateau).decouverte(plateau.cards_dragage[0], action - 241, type_dec="dragage")
            elif plateau.cards_entrave:
                TourEnCours(plateau).decouverte(plateau.cards_entrave[0], action - 241, type_dec="entrave")
        elif action < 255:
            if generate_logs:
            ...
            if generate_logs:
                action_line["action"] = "lieu_utilisé"
            if (action - 255) % 15 == 0:
                target = player.hero
            elif (action - 255) % 15 < 7:
                target = player.servants[(action - 256) % 15]
            elif (action - 255) % 15 == 7:
                target = adv.hero
            else:
                target = adv.servants[(action - 263) % 15]
            TourEnCours(plateau).util_lieu(player.lieux[(action - 255) // 15], target)
