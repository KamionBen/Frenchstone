    ...
    def apply_effects(self, carte, target=None):
        ...
        if "Gardien du temps" in player.permanent_buff:
            if "Dragon" in carte.genre:
                ...
                print("Gardien du temps ?", carte)
        if "regis" in player.permanent_buff and player.permanent_buff["regis"] == 1 and carte.name == "Recrue de la main d'argent":
            carte.attack += 3
            carte.base_attack += 3
            carte.health += 3
            carte.base_health += 3
            carte.effects["rale d'agonie"] = ["regis", ["allié"], 1]
        ...
        if carte.type == "Serviteur" and not "titan" in carte.effects:
            ...
            if "cri de guerre" in carte.effects and not (carte.is_dead() or "rale_applied" in carte.effects):
                    ...
                    if target is not None:
                        ...
                        if "self" in carte.effects["cri de guerre"][1]:
                            if carte.name == "Mousse de la voile sanglante" :
                                ...
                                carte.attack += 2 * player.etres_terrestres
                                carte.base_attack += 2 * player.etres_terrestres
                                carte.health += 2 * player.etres_terrestres
                                carte.base_health += 2 * player.etres_terrestres
                ...
                if "add_hand" in carte.effects["cri de guerre"]:
                    ...
                    if "addcost_2" in carte.effects["cri de guerre"][1]:
                        for spell in [x for x in player.hand if x.type == "Sort"]:
                            ...
                            new_spell = Card(**random.choice([x for x in all_spells if x["decouvrable"] == 1 and x["cost"] == min(10, player.hand.cards[index_spell].cost + 2)]))
                            new_spell.cost -= 2
                            ...
                            for invoked_servant in carte.effects["cri de guerre"][2]:
                                if len(player.servants) + len(player.lieux) < 7:
                                    ...
                                    if "until_cadavre" in carte.effects["cri de guerre"][1]:
                                        ...
                                        real_servant.attack += carte.effects["cri de guerre"][1][-1][0]
                                        real_servant.base_attack += carte.effects["cri de guerre"][1][-1][0]
                                        real_servant.health += carte.effects["cri de guerre"][1][-1][1]
                                        real_servant.base_health += carte.effects["cri de guerre"][1][-1][1]
                                        if "bouclier divin" in carte.effects["cri de guerre"][1]:
                ...
                if "copy" in carte.effects["cri de guerre"] and target is not None:
                    ...
                    self.invoke_servant(copy_card, player)
            ...
            if target is not None:
                if "decouverte" in target:
                    ...
                    for serv in target[1]:
                        if len(player.servants) + len(player.lieux) < 7:
                            self.invoke_servant(get_card(serv, all_servants), player)
            ...
            elif "equip_arme" in carte.effects:
                weapon = get_card(carte.effects["equip_arme"][-1], all_weapons)
                if player.weapon is not None:
                    player.weapon.health = 0
                    ...
                    if "repeat_if_cadavre_and_serv" in carte.effects and player.cadavres > 0 and len(player.servants.cards + adv.servants.cards) > 0:
                        ...
                        if "double" in carte.effects["damage"][0]:
                            for creature in player.servants.cards + adv.servants.cards:
                                ...
                                TourEnCours(self.plt).apply_effects(carte)
                                creature.damage(carte.effects["damage"][-1] + player.spell_damage, toxic=True if "toxicite" in carte.effects else False)
                        elif "ennemi" in carte.effects["damage"][0] and adv.servants:
                    ...
                    if "aléatoire" in carte.effects["add_hand"][1]:
                        if "serviteur" in carte.effects["add_hand"][1]:
                            if "colossal" in carte.effects["add_hand"][1]:
                                ...
                                player.hand.add(card_to_add)
                            if "Élémentaire" in carte.effects["add_hand"][1] and "123" in carte.effects["add_hand"][1]:
                                for i in range(3):
                            ...
                            if "until_full" in carte.effects["add_hand"][1]:
                                ...
                                if "reduc" in carte.effects["add_hand"][1]:
                                    for _ in range(carte.effects["add_hand"][1][-1]):
                                        card_to_add = Card(**random.choice([x for x in all_spells if x["decouvrable"] == 1 and x["classe"] == "Mage" and x["name"] != carte.name]))
            ...
            if "invocation" in carte.effects:
                ...
                if "conditional" in carte.effects["invocation"]:
                    if "no_serv_deck" in carte.effects["invocation"]:
                        ...
                        if player.dead_demons:
                            ...
                            carte.effects["invocation"] = []
                    elif "ecoles_jouees" in carte.effects["invocation"] and player.ecoles_jouees:
                ...
                to_invoke = get_card(target.name, all_servants)
                to_invoke.attack = target.attack
                to_invoke.base_attack = target.base_attack
                to_invoke.health = target.health
                to_invoke.blessure = target.blessure
                to_invoke.base_health = target.base_health
                to_invoke.effects = target.effects
                to_invoke.remaining_atk = 0
                ...
                if "invocation" in carte.effects["rale d'agonie"] and len(player.servants) + len(player.lieux) < 7:
                    ...
                    if "serviteur" in carte.effects["rale d'agonie"][1] and "allié" in carte.effects["rale d'agonie"][1] and "aléatoire" in carte.effects["rale d'agonie"][1]:
                        if player.servants.cards:
                            ...
                            boosted_serv.attack += carte.effects["rale d'agonie"][2][0]
                            boosted_serv.base_attack += carte.effects["rale d'agonie"][2][0]
                            boosted_serv.health += carte.effects["rale d'agonie"][2][0]
                            boosted_serv.base_health += carte.effects["rale d'agonie"][2][0]
        ...
        if type(attaquant) in (Player, Card) and type(cible) in (Player, Card):
            ...
            if not attaquant.is_dead() and attaquant in player.servants.cards + adv.servants.cards + [player] + [adv]:
                if ([x for x in player.attached if x[0] == "Aura des croises"] and attaquant in player.servants.cards) or ([x for x in adv.attached if x[0] == "Aura des croises"] and attaquant in adv.servants.cards):
                    attaquant.attack += 2
                    attaquant.base_attack += 2
                    attaquant.health += 1
                    attaquant.base_health += 1
    ...
    def choice_decouverte(self, carte, type=None, genre=None, classe=None, other=None, card_group=None, reduc=None):
        ...
        if type == "serviteur":
            ...
            if other == "vert":
                ...
                group_filtered = [x for x in all_cards if x["decouvrable"] == 1 and x["name"] != carte.name and x["classe"] == classe]
            else:
        ...
        if type_dec == "decouverte":
            ...
            try:
                ...
                if player.last_card.name == "Sauveteuse des grands fonds" and cards[choice].type == "Serviteur":
                    cards[choice].attack += player.last_card.attack
                    cards[choice].base_attack += player.last_card.base_attack
                    cards[choice].health += player.last_card.health
                    cards[choice].base_health += player.last_card.base_health
